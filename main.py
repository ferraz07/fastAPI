...
class ConnectionManager:
    def __init__(self):
        # Dicionário para manter as conexões ativas: {usuario_id: websocket}
...
#NOVO
@app.get("/usuarios/{usuario_id}/conversas", tags=["Chat"])
def listar_conversas(usuario_id: int):
    conn = get_connection()
    cursor = conn.cursor()
    try:
        sql = """
            ...
            FROM Conversa C
            INNER JOIN Medico M ON C.MedicoUsuarioID = M.UsuarioID
            INNER JOIN Paciente P ON C.PacienteUsuarioID = P.UsuarioID
